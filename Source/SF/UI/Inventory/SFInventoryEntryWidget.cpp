#include "SFInventoryEntryWidget.h"
...
USFInventoryEntryWidget::USFInventoryEntryWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void USFInventoryEntryWidget::Init(USFInventorySlotWidget* InSlotWidget, USFItemInstance* InItemInstance, int32 InItemCount)
{
...
FReply USFInventoryEntryWidget::NativeOnMouseButtonDown(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void USFInventoryEntryWidget::NativeOnDragDetected(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent, UDragDropOperation*& OutOperation)
{
	...
	// From 슬롯 설정
	DragDrop->FromSlot = FSFItemSlotHandle(ESFItemSlotType::Inventory, OwningSlotWidget->GetSlotIndex());
